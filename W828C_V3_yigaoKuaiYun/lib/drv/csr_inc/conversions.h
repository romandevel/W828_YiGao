...
#define GET_PACKED_MA_UNIDATA_REQUEST_FRAME_PRIORITY(_buf)              \
    COAL_GET_UINT32_FROM_LITTLE_ENDIAN(((_buf) + SIZEOF_SIGNAL_HEADER + UNIFI_MAX_DATA_REFERENCES*SIZEOF_DATAREF + 14))

#define GET_PACKED_MA_PACKET_REQUEST_FRAME_PRIORITY(_buf)              \
    COAL_GET_UINT32_FROM_LITTLE_ENDIAN(((_buf) + SIZEOF_SIGNAL_HEADER + UNIFI_MAX_DATA_REFERENCES*SIZEOF_DATAREF + 6))

CsrInt32 get_packed_struct_size(CsrUint8 *buf);
CsrInt32 read_unpack_signal(CsrUint8 *ptr, CSR_SIGNAL *sig);
CsrInt32 write_pack(CSR_SIGNAL *sig, CsrUint8 *ptr, CsrUint16 *sig_len);
