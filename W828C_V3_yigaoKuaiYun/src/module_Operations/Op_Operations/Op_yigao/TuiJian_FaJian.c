...
/*==========================================到件界面==========================================*/
const tGrapViewAttr YGTuiJian_FaJian_ViewAttr[] =
{
...
const tGrapEditPriv YGTuiJian_FaJian_EditPriv[] = 
{
	...
	{gtOpInfoGlb.station2,0x0,0,0,2,4,12,  1, COMPANY_NAME_LEN-1,		0,0,0,0,0,0,  50,50,0,  1,0,0,0},//
};


int YGTuiJian_FaJian_BtnPress(void *pView, U16 state)
{
	tGRAPDESKBOX	*pDesk;
	tGRAPBUTTON 	*pBtn;
	tDATA_INFO 		Recd;
	...
	if(state == STATE_NORMAL)
	{
		...
		{
		case BUTTON_ID_1://确定
			if(gpEdit_Stat2->view_attr.curFocus == 1)
			{
...
U32 YGTuiJian_FaJian_EditKeyEnd(void *pView, U8 type)
{
	tGRAPDESKBOX	*pDesk;
...
S32 YGTuiJian_FaJian_EditEnter(void *pView)
{
	tGRAPDESKBOX	*pDesk;
...
/*-----------------------------------------------
*函数:Operation_YGTuiJian_FaJian_Desk
*功能:收件
*参数:
*返回:
*-----------------------------------------------*/
int Operation_YGTuiJian_FaJian_Desk(void *pDeskFather)
{
	tGRAPDESKBOX	*pDesk;
	tGRAPBUTTON		*pBtn;
	tGRAPEDIT		*pEdit;
	...
	pDesk = Grap_CreateDeskbox((tGrapViewAttr*)&YGTuiJian_FaJian_ViewAttr[idx++], "退件发件");
	...
	for (i=0; i<5; i++)
	{
		pBtn = Grap_InsertButton(pDesk, &YGTuiJian_FaJian_ViewAttr[idx++], &YGTuiJian_FaJian_BtnPriv[i]);
		if(NULL == pBtn ) return -1;
		
		if(i < 3)
		{
			pBtn->pressEnter = YGTuiJian_FaJian_BtnPress;
		}
	}
	...
	for (i=0; i<3; i++)
	{
		pEdit = Grap_InsertEdit(pDesk, &YGTuiJian_FaJian_ViewAttr[idx++], &YGTuiJian_FaJian_EditPriv[i]);
		pEdit->DrawInputCnt = OpCom_EditDrawInputCnt;
		if(NULL == pEdit ) return -1;
		if(i == 0)//下一站
		{
			
			pEdit->editKeyEnd = YGTuiJian_FaJian_EditKeyEnd;
			gpEdit_Stat2 = pEdit;
		}
		else if(i == 1)//运单号
		{
		...
		{
			pEdit->editenter  = YGTuiJian_FaJian_EditEnter;
		}
	}
...
/*-----------------------------------------记录查询显示------------------------------------------------*/
const U8* const opType_YGTuiJian_FaJian[]=//发件
{
	"状态:",
...
/*---------------------------------------------------------------
*函数: YGTuiJian_FaJian_SearchDisplay
*功能: 显示记录查询出来的字符串
*参数: pData:查询到的结果
       total:查询到的总数
       pDispStr:(out)显示的buf
...
*返回: 
----------------------------------------------------------------*/
int YGTuiJian_FaJian_SearchDisplay(void *p)
{
...
%s%s\r\n%s%s\r\n-----------------------\r\n",
							opType_YGTuiJian_FaJian[0], str[stat],
							opType_YGTuiJian_FaJian[1], pComm->opType,
							opType_YGTuiJian_FaJian[2], pComm->scanStat,
							opType_YGTuiJian_FaJian[3], pComm->userId,
							opType_YGTuiJian_FaJian[4], pComm->scanTime,
							
							opType_YGTuiJian_FaJian[5], saomiao->nextstat,
							opType_YGTuiJian_FaJian[6], pComm->code
							);
...
*返回: 0:成功   -1:网络通讯异常
----------------------------------------------------------------*/
int YGTuiJian_FaJian_FillData(void *p)
{
	...
	sprintf(pSend, "StrJson=[Request][data][no Code='06'][a]%s[/a][b]%s[/b][c]%s[/c][d]%s[/d][e]%s[/e][f]%s[/f][/no][/data][/Request]&Checking_Key=002",
				pComm->userId,pComm->scanStat,gtHyc.MachineCode,pSaomiao->nextstat,pComm->code,pComm->scanTime);
