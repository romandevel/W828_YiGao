...
/*==========================================到件界面==========================================*/
const tGrapViewAttr YGTuiJian_ShouJian_ViewAttr[] =
{
...
const tGrapButtonPriv YGTuiJian_ShouJian_BtnPriv[]=
{
#ifdef W818C
...
const tGrapEditPriv YGTuiJian_ShouJian_EditPriv[] = 
{
...
int YGTuiJian_ShouJian_BtnPress(void *pView, U16 state)
{
	tGRAPDESKBOX	*pDesk;
	tGRAPBUTTON 	*pBtn;
	tDATA_INFO 		Recd;
...
U32 YGTuiJian_ShouJian_EditKeyEnd(void *pView, U8 type)
{
	tGRAPDESKBOX	*pDesk;
...
S32 YGTuiJian_ShouJian_EditEnter(void *pView)
{
	tGRAPDESKBOX	*pDesk;
...
/*-----------------------------------------------
*函数:Operation_YGTuiJian_ShouJian_Desk
*功能:收件
*参数:
*返回:
*-----------------------------------------------*/
int Operation_YGTuiJian_ShouJian_Desk(void *pDeskFather)
{
	tGRAPDESKBOX	*pDesk;
	tGRAPBUTTON		*pBtn;
	tGRAPEDIT		*pEdit;
	...
	pDesk = Grap_CreateDeskbox((tGrapViewAttr*)&YGTuiJian_ShouJian_ViewAttr[idx++], "退件收件");
	...
	for (i=0; i<5; i++)
	{
		pBtn = Grap_InsertButton(pDesk, &YGTuiJian_ShouJian_ViewAttr[idx++], &YGTuiJian_ShouJian_BtnPriv[i]);
		if(NULL == pBtn ) return -1;
		
		if(i < 3)
		{
			pBtn->pressEnter = YGTuiJian_ShouJian_BtnPress;
		}
	}
	...
	for (i=0; i<3; i++)
	{
		pEdit = Grap_InsertEdit(pDesk, &YGTuiJian_ShouJian_ViewAttr[idx++], &YGTuiJian_ShouJian_EditPriv[i]);
		pEdit->DrawInputCnt = OpCom_EditDrawInputCnt;
		if(NULL == pEdit ) return -1;
		if(i == 0)//上一站
		{
			
			pEdit->editKeyEnd = YGTuiJian_ShouJian_EditKeyEnd;
			gpEdit_Stat1 = pEdit;
		}
		else if(i == 1)//运单号
		{
		...
		{
			pEdit->editenter  = YGTuiJian_ShouJian_EditEnter;
		}
	}
...
/*-----------------------------------------记录查询显示------------------------------------------------*/
const U8* const opType_YGTuiJian_ShouJian[]=//发件
{
	"状态:",
...
/*---------------------------------------------------------------
*函数: YGTuiJian_ShouJian_SearchDisplay
*功能: 显示记录查询出来的字符串
*参数: pData:查询到的结果
       total:查询到的总数
       pDispStr:(out)显示的buf
...
*返回: 
----------------------------------------------------------------*/
int YGTuiJian_ShouJian_SearchDisplay(void *p)
{
...
%s%s\r\n%s%s\r\n-----------------------\r\n",
							opType_YGTuiJian_ShouJian[0], str[stat],
							opType_YGTuiJian_ShouJian[1], pComm->opType,
							opType_YGTuiJian_ShouJian[2], pComm->scanStat,
							opType_YGTuiJian_ShouJian[3], pComm->userId,
							opType_YGTuiJian_ShouJian[4], pComm->scanTime,
							
							opType_YGTuiJian_ShouJian[5], saomiao->prevstat,
							opType_YGTuiJian_ShouJian[6], pComm->code
							);
...
/*-----------------------------------------组织请求数据------------------------------------------------*/
/*---------------------------------------------------------------
*函数: YGTuiJian_ShouJian_FillData
*功能: 填充请求数据
*参数: pSend:用于发送的buf
       pNode:准备打包发送的数据
       cnt:准备打包的个数
...
*返回: 0:成功   -1:网络通讯异常
----------------------------------------------------------------*/
int YGTuiJian_ShouJian_FillData(void *p)
{
	...
	sprintf(pSend, "StrJson=[Request][data][no Code='04'][a]%s[/a][b]%s[/b][c]%s[/c][d]%s[/d][e]%s[/e][f]%s[/f][/no][/data][/Request]&Checking_Key=002",
