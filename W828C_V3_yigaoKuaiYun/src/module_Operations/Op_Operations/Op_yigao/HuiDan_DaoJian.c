...
/*==========================================到件界面==========================================*/
const tGrapViewAttr YGHuiDan_DaoJian_ViewAttr[] =
{
...
const tGrapButtonPriv YGHuiDan_DaoJian_BtnPriv[]=
{
#ifdef W818C
...
const tGrapEditPriv YGHuiDan_DaoJian_EditPriv[] = 
{
...
int YGHuiDan_DaoJian_BtnPress(void *pView, U16 state)
{
	tGRAPDESKBOX	*pDesk;
	tGRAPBUTTON 	*pBtn;
	tDATA_INFO 		Recd;
...
U32 YGHuiDan_DaoJian_EditKeyEnd(void *pView, U8 type)
{
	tGRAPDESKBOX	*pDesk;
...
S32 YGHuiDan_DaoJian_EditEnter(void *pView)
{
	tGRAPDESKBOX	*pDesk;
...
/*-----------------------------------------------
*函数:Operation_YGHuiDan_DaoJian_Desk
*功能:收件
*参数:
*返回:
*-----------------------------------------------*/
int Operation_YGHuiDan_DaoJian_Desk(void *pDeskFather)
{
	tGRAPDESKBOX	*pDesk;
	tGRAPBUTTON		*pBtn;
	tGRAPEDIT		*pEdit;
	...
	pDesk = Grap_CreateDeskbox((tGrapViewAttr*)&YGHuiDan_DaoJian_ViewAttr[idx++], "回单到件");
	...
	for (i=0; i<5; i++)
	{
		pBtn = Grap_InsertButton(pDesk, &YGHuiDan_DaoJian_ViewAttr[idx++], &YGHuiDan_DaoJian_BtnPriv[i]);
		if(NULL == pBtn ) return -1;
		
		if(i < 3)
		{
			pBtn->pressEnter = YGHuiDan_DaoJian_BtnPress;
		}
	}
	...
	for (i=0; i<3; i++)
	{
		pEdit = Grap_InsertEdit(pDesk, &YGHuiDan_DaoJian_ViewAttr[idx++], &YGHuiDan_DaoJian_EditPriv[i]);
		pEdit->DrawInputCnt = OpCom_EditDrawInputCnt;
		if(NULL == pEdit ) return -1;
		if(i == 0)//上一站
		{
			
			pEdit->editKeyEnd = YGHuiDan_DaoJian_EditKeyEnd;
			gpEdit_Stat1 = pEdit;
		}
		else if(i == 1)//运单号
		{
		...
		{
			pEdit->editenter  = YGHuiDan_DaoJian_EditEnter;
		}
	}
...
/*-----------------------------------------记录查询显示------------------------------------------------*/
const U8* const opType_YGHuiDan_DaoJian[]=//发件
{
	"状态:",
...
/*---------------------------------------------------------------
*函数: YGHuiDan_DaoJian_SearchDisplay
*功能: 显示记录查询出来的字符串
*参数: pData:查询到的结果
       total:查询到的总数
       pDispStr:(out)显示的buf
...
*返回: 
----------------------------------------------------------------*/
int YGHuiDan_DaoJian_SearchDisplay(void *p)
{
...
%s%s\r\n%s%s\r\n-----------------------\r\n",
							opType_YGHuiDan_DaoJian[0], str[stat],
							opType_YGHuiDan_DaoJian[1], pComm->opType,
							opType_YGHuiDan_DaoJian[2], pComm->scanStat,
							opType_YGHuiDan_DaoJian[3], pComm->userId,
							opType_YGHuiDan_DaoJian[4], pComm->scanTime,
							
							opType_YGHuiDan_DaoJian[5], saomiao->prevstat,
							opType_YGHuiDan_DaoJian[6], pComm->code
							);
...
*返回: 0:成功   -1:网络通讯异常
----------------------------------------------------------------*/
int YGHuiDan_DaoJian_FillData(void *p)
{
	...
	sprintf(pSend, "StrJson=[Request][data][no Code='11'][a]%s[/a][b]%s[/b][c]%s[/c][d]%s[/d][e]%s[/e][f]%s[/f][/no][/data][/Request]&Checking_Key=002",
