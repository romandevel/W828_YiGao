...
/*==========================================到件界面==========================================*/
const tGrapViewAttr YGTuiJian_DaoJian_ViewAttr[] =
{
...
const tGrapButtonPriv YGTuiJian_DaoJian_BtnPriv[]=
{
#ifdef W818C
...
const tGrapEditPriv YGTuiJian_DaoJian_EditPriv[] = 
{
...
int YGTuiJian_DaoJian_BtnPress(void *pView, U16 state)
{
	tGRAPDESKBOX	*pDesk;
	tGRAPBUTTON 	*pBtn;
	tDATA_INFO 		Recd;
...
U32 YGTuiJian_DaoJian_EditKeyEnd(void *pView, U8 type)
{
	tGRAPDESKBOX	*pDesk;
...
S32 YGTuiJian_DaoJian_EditEnter(void *pView)
{
	tGRAPDESKBOX	*pDesk;
...
/*-----------------------------------------------
*函数:Operation_YGTuiJian_DaoJian_Desk
*功能:收件
*参数:
*返回:
*-----------------------------------------------*/
int Operation_YGTuiJian_DaoJian_Desk(void *pDeskFather)
{
	tGRAPDESKBOX	*pDesk;
	tGRAPBUTTON		*pBtn;
	tGRAPEDIT		*pEdit;
	...
	pDesk = Grap_CreateDeskbox((tGrapViewAttr*)&YGTuiJian_DaoJian_ViewAttr[idx++], "退件到件");
	...
	for (i=0; i<5; i++)
	{
		pBtn = Grap_InsertButton(pDesk, &YGTuiJian_DaoJian_ViewAttr[idx++], &YGTuiJian_DaoJian_BtnPriv[i]);
		if(NULL == pBtn ) return -1;
		
		if(i < 3)
		{
			pBtn->pressEnter = YGTuiJian_DaoJian_BtnPress;
		}
	}
	...
	for (i=0; i<3; i++)
	{
		pEdit = Grap_InsertEdit(pDesk, &YGTuiJian_DaoJian_ViewAttr[idx++], &YGTuiJian_DaoJian_EditPriv[i]);
		pEdit->DrawInputCnt = OpCom_EditDrawInputCnt;
		if(NULL == pEdit ) return -1;
		if(i == 0)//上一站
		{
			
			pEdit->editKeyEnd = YGTuiJian_DaoJian_EditKeyEnd;
			gpEdit_Stat1 = pEdit;
		}
		else if(i == 1)//运单号
		{
		...
		{
			pEdit->editenter  = YGTuiJian_DaoJian_EditEnter;
		}
	}
...
/*-----------------------------------------记录查询显示------------------------------------------------*/
const U8* const opType_YGTuiJian_DaoJian[]=//发件
{
	"状态:",
...
/*---------------------------------------------------------------
*函数: YGTuiJian_DaoJian_SearchDisplay
*功能: 显示记录查询出来的字符串
*参数: pData:查询到的结果
       total:查询到的总数
       pDispStr:(out)显示的buf
...
*返回: 
----------------------------------------------------------------*/
int YGTuiJian_DaoJian_SearchDisplay(void *p)
{
...
%s%s\r\n%s%s\r\n-----------------------\r\n",
							opType_YGTuiJian_DaoJian[0], str[stat],
							opType_YGTuiJian_DaoJian[1], pComm->opType,
							opType_YGTuiJian_DaoJian[2], pComm->scanStat,
							opType_YGTuiJian_DaoJian[3], pComm->userId,
							opType_YGTuiJian_DaoJian[4], pComm->scanTime,
							
							opType_YGTuiJian_DaoJian[5], saomiao->prevstat,
							opType_YGTuiJian_DaoJian[6], pComm->code
							);
...
*返回: 0:成功   -1:网络通讯异常
----------------------------------------------------------------*/
int YGTuiJian_DaoJian_FillData(void *p)
{
	...
	sprintf(pSend, "StrJson=[Request][data][no Code='10'][a]%s[/a][b]%s[/b][c]%s[/c][d]%s[/d][e]%s[/e][f]%s[/f][/no][/data][/Request]&Checking_Key=002",
