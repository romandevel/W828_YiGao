...
/*==========================================到件界面==========================================*/
const tGrapViewAttr YGFaJian_ViewAttr[] =
{
...
const tGrapButtonPriv YGFaJian_BtnPriv[]=
{
#ifdef W818C
...
const tGrapEditPriv YGFaJian_EditPriv[] = 
{
...
int YGFaJian_BtnPress(void *pView, U16 state)
{
	tGRAPDESKBOX	*pDesk;
	tGRAPBUTTON 	*pBtn;
	tDATA_INFO 		Recd;
...
U32 YGFaJian_EditKeyEnd(void *pView, U8 type)
{
	tGRAPDESKBOX	*pDesk;
...
S32 YGFaJian_EditEnter(void *pView)
{
	tGRAPDESKBOX	*pDesk;
...
/*-----------------------------------------------
*函数:Operation_YGFaJian_Desk
*功能:收件
*参数:
*返回:
*-----------------------------------------------*/
int Operation_YGFaJian_Desk(void *pDeskFather)
{
	tGRAPDESKBOX	*pDesk;
	tGRAPBUTTON		*pBtn;
	tGRAPEDIT		*pEdit;
	...
	pDesk = Grap_CreateDeskbox((tGrapViewAttr*)&YGFaJian_ViewAttr[idx++], "发件操作");
	...
	for (i=0; i<5; i++)
	{
		pBtn = Grap_InsertButton(pDesk, &YGFaJian_ViewAttr[idx++], &YGFaJian_BtnPriv[i]);
		if(NULL == pBtn ) return -1;
		
		if(i < 3)
		{
			pBtn->pressEnter = YGFaJian_BtnPress;
		}
	}
	...
	for (i=0; i<3; i++)
	{
		pEdit = Grap_InsertEdit(pDesk, &YGFaJian_ViewAttr[idx++], &YGFaJian_EditPriv[i]);
		pEdit->DrawInputCnt = OpCom_EditDrawInputCnt;
		if(NULL == pEdit ) return -1;
		if(i == 0)//下一站
		{
			pEdit->editKeyEnd = YGFaJian_EditKeyEnd;
			gpEdit_Stat2 = pEdit;
		}
		else if(i == 1)//运单号
		{
		...
		{
			pEdit->editenter  = YGFaJian_EditEnter;
		}
	}
...
/*-----------------------------------------记录查询显示------------------------------------------------*/
const U8* const opType_YGFaJian[]=//发件
{
	"状态:",
...
*返回: 
----------------------------------------------------------------*/
int YGFaJian_SearchDisplay(void *p)
{
...
%s%s\r\n%s%s\r\n-----------------------\r\n",
							opType_YGFaJian[0], str[stat],
							opType_YGFaJian[1], pComm->opType,
							opType_YGFaJian[2], pComm->scanStat,
							opType_YGFaJian[3], pComm->userId,
							opType_YGFaJian[4], pComm->scanTime,
							
							opType_YGFaJian[5], saomiao->nextstat,
							opType_YGFaJian[6], pComm->code
							);
...
int YGFaJian_FillData(void *p)
{
	...
	sprintf(pSend, "StrJson=[Request][data][no Code='05'][a]%s[/a][b]%s[/b][c]%s[/c][d]%s[/d][e]%s[/e][f]%s[/f][/no][/data][/Request]&Checking_Key=002",
