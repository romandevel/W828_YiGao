...
/*==========================================到件界面==========================================*/
const tGrapViewAttr YGHuiDan_FaJian_ViewAttr[] =
{
...
const tGrapButtonPriv YGHuiDan_FaJian_BtnPriv[]=
{
#ifdef W818C
...
const tGrapEditPriv YGHuiDan_FaJian_EditPriv[] = 
{
...
int YGHuiDan_FaJian_BtnPress(void *pView, U16 state)
{
	tGRAPDESKBOX	*pDesk;
	tGRAPBUTTON 	*pBtn;
	tDATA_INFO 		Recd;
...
U32 YGHuiDan_FaJian_EditKeyEnd(void *pView, U8 type)
{
	tGRAPDESKBOX	*pDesk;
...
S32 YGHuiDan_FaJian_EditEnter(void *pView)
{
	tGRAPDESKBOX	*pDesk;
...
/*-----------------------------------------------
*函数:Operation_YGHuiDan_FaJian_Desk
*功能:收件
*参数:
*返回:
*-----------------------------------------------*/
int Operation_YGHuiDan_FaJian_Desk(void *pDeskFather)
{
	tGRAPDESKBOX	*pDesk;
	tGRAPBUTTON		*pBtn;
	tGRAPEDIT		*pEdit;
	...
	pDesk = Grap_CreateDeskbox((tGrapViewAttr*)&YGHuiDan_FaJian_ViewAttr[idx++], "回单发件");
	...
	for (i=0; i<5; i++)
	{
		pBtn = Grap_InsertButton(pDesk, &YGHuiDan_FaJian_ViewAttr[idx++], &YGHuiDan_FaJian_BtnPriv[i]);
		if(NULL == pBtn ) return -1;
		
		if(i < 3)
		{
			pBtn->pressEnter = YGHuiDan_FaJian_BtnPress;
		}
	}
	...
	for (i=0; i<3; i++)
	{
		pEdit = Grap_InsertEdit(pDesk, &YGHuiDan_FaJian_ViewAttr[idx++], &YGHuiDan_FaJian_EditPriv[i]);
		pEdit->DrawInputCnt = OpCom_EditDrawInputCnt;
		if(NULL == pEdit ) return -1;
		if(i == 0)//下一站
		{
			
			pEdit->editKeyEnd = YGHuiDan_FaJian_EditKeyEnd;
			gpEdit_Stat2 = pEdit;
		}
		else if(i == 1)//运单号
		{
		...
		{
			pEdit->editenter  = YGHuiDan_FaJian_EditEnter;
		}
	}
...
/*-----------------------------------------记录查询显示------------------------------------------------*/
const U8* const opType_YGHuiDan_FaJian[]=//发件
{
	"状态:",
...
/*---------------------------------------------------------------
*函数: YGHuiDan_FaJian_SearchDisplay
*功能: 显示记录查询出来的字符串
*参数: pData:查询到的结果
       total:查询到的总数
       pDispStr:(out)显示的buf
...
*返回: 
----------------------------------------------------------------*/
int YGHuiDan_FaJian_SearchDisplay(void *p)
{
...
%s%s\r\n%s%s\r\n-----------------------\r\n",
							opType_YGHuiDan_FaJian[0], str[stat],
							opType_YGHuiDan_FaJian[1], pComm->opType,
							opType_YGHuiDan_FaJian[2], pComm->scanStat,
							opType_YGHuiDan_FaJian[3], pComm->userId,
							opType_YGHuiDan_FaJian[4], pComm->scanTime,
							
							opType_YGHuiDan_FaJian[5], saomiao->nextstat,
							opType_YGHuiDan_FaJian[6], pComm->code
							);
...
*返回: 0:成功   -1:网络通讯异常
----------------------------------------------------------------*/
int YGHuiDan_FaJian_FillData(void *p)
{
	...
	sprintf(pSend, "StrJson=[Request][data][no Code='07'][a]%s[/a][b]%s[/b][c]%s[/c][d]%s[/d][e]%s[/e][f]%s[/f][/no][/data][/Request]&Checking_Key=002",
