...
/*==========================================收件界面==========================================*/
const tGrapViewAttr YGShouJian_ViewAttr[] =
{
...
const tGrapButtonPriv YGShouJian_BtnPriv[]=
{
#ifdef W818C
...
const tGrapEditPriv YGShouJian_EditPriv[] = 
{
...
int YGShouJian_BtnPress(void *pView, U16 state)
{
	tGRAPDESKBOX	*pDesk;
	tGRAPBUTTON 	*pBtn;
	tDATA_INFO 		Recd;
...
U32 YGShouJian_EditKeyEnd(void *pView, U8 type)
{
	tGRAPDESKBOX	*pDesk;
...
S32 YGShouJian_EditEnter(void *pView)
{
	tGRAPDESKBOX	*pDesk;
	tGRAPEDIT 		*pEdit;
	int ret;
...
/*-----------------------------------------------
*函数:Operation_YGShouJian_Desk
*功能:收件
*参数:
*返回:
*-----------------------------------------------*/
int Operation_YGShouJian_Desk(void *pDeskFather)
{
	tGRAPDESKBOX	*pDesk;
	tGRAPBUTTON		*pBtn;
	tGRAPEDIT		*pEdit;
	...
	pDesk = Grap_CreateDeskbox((tGrapViewAttr*)&YGShouJian_ViewAttr[idx++], "收件扫描");
	...
	for (i=0; i<5; i++)
	{
		pBtn = Grap_InsertButton(pDesk, &YGShouJian_ViewAttr[idx++], &YGShouJian_BtnPriv[i]);
		if(NULL == pBtn ) return -1;
		
		if(i < 3)
		{
			pBtn->pressEnter = YGShouJian_BtnPress;
		}
	}
	...
	for (i=0; i<3; i++)
	{
		pEdit = Grap_InsertEdit(pDesk, &YGShouJian_ViewAttr[idx++], &YGShouJian_EditPriv[i]);
		pEdit->DrawInputCnt = OpCom_EditDrawInputCnt;
		if(NULL == pEdit ) return -1;
		if(i == 0)//收件员
		{
			pEdit->editenter  = YGShouJian_EditEnter;
			pEdit->editKeyEnd = YGShouJian_EditKeyEnd;
...
/*-----------------------------------------记录查询显示------------------------------------------------*/
const U8* const opType_YGshoujian[]=//收件
{
	"状态:",
...
*返回: 
----------------------------------------------------------------*/
int YGShouJian_SearchDisplay(void *p)
{
...
%s%s\r\n%s%s\r\n-----------------------\r\n",
							opType_YGshoujian[0], str[stat],
							opType_YGshoujian[1], pComm->opType,
							opType_YGshoujian[2], pComm->scanStat,
							opType_YGshoujian[3], pComm->userId,
							//opType_YGshoujian[4], pComm->groupId,
							opType_YGshoujian[4], pComm->scanTime,
							
							opType_YGshoujian[5], saomiao->deliverer,
							opType_YGshoujian[6], pComm->code
							);
...
*返回: 0:成功   -1:网络通讯异常
----------------------------------------------------------------*/
int YGShouJian_FillData(void *p)
{
	...
	sprintf(pSend, "StrJson=[Request][data][no Code='03'][a]%s[/a][b]%s[/b][c]%s[/c][d]%s[/d][e]%s[/e][f]%s[/f][/no][/data][/Request]&Checking_Key=002",
				pComm->userId,pComm->scanStat,gtHyc.MachineCode,saomiao->deliverer,pComm->code,pComm->scanTime);
	
	//sprintf(pSend, "StrJson=[Request][head][a]%s[/a][b]%s[/b][c]%s[/c][/head][data][no Code='03'][d]%s[/d][e]%s[/e][f]%s[/f][/no][/data][/Request]&Checking_Key=002",
	//			pComm->userId,pComm->scanStat,gtHyc.MachineCode,saomiao->deliverer,pComm->code,pComm->scanTime);
