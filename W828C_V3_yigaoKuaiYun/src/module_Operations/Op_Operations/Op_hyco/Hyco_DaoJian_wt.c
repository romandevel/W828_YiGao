...
/*==========================================到件界面==========================================*/
const tGrapViewAttr DaoJianWT_ViewAttr[] =
{
...
#endif	
	{BUTTON_ID_4,	   140,61, 90,20,   140,61,238,81,   YD_AN1_EN,YD_AN2_EN,0,0,1,0,0,0},//蓝牙称重
	...
	{EDIT_ID_3,	   55,85,172,22,   55,85,227,107, 0,0,0xffff,0,1,0,1,0},//运单号
	
	{EDIT_ID_20,   140,35 ,100,22,   0,0,0,0,     0,0,0xCE59,0,1,0,0,0},
};

const tGrapButtonPriv DaoJianWT_BtnPriv[]=
{
#ifdef W818C
...
const tGrapEditPriv DaoJianWT_EditPriv[] = 
{
...
int DaoJianWT_BtnPress(void *pView, U16 state)
{
	tGRAPDESKBOX	*pDesk;
	tGRAPBUTTON 	*pBtn;
	tGRAPEDIT 		*pEditWT;
...
U32 DaoJianWT_EditKeyEnd(void *pView, U8 type)
{
	tGRAPDESKBOX	*pDesk;
...
S32 DaoJianWT_EditEnter(void *pView)
{
	tGRAPDESKBOX	*pDesk;
	tGRAPEDIT 		*pEdit;
	U8 *pCode;
...
/*-----------------------------------------------
*函数:Operation_DaoJianWT_Desk
*功能:收件
*参数:
*返回:
*-----------------------------------------------*/
int Operation_DaoJianWT_Desk(void *pDeskFather)
{
	tGRAPDESKBOX	*pDesk;
	tGRAPBUTTON		*pBtn;
	tGRAPEDIT		*pEdit;
	...
	pDesk = Grap_CreateDeskbox((tGrapViewAttr*)&DaoJianWT_ViewAttr[idx++], "到件称重");
	...
	for (i=0; i<8; i++)
	{
		pBtn = Grap_InsertButton(pDesk, &DaoJianWT_ViewAttr[idx++], &DaoJianWT_BtnPriv[i]);
		if(NULL == pBtn ) return -1;
		
		if(i < 4)
		{
			pBtn->pressEnter = DaoJianWT_BtnPress;
			if(i == 3)
			{
	...
	for (i=0; i<4; i++)
	{
		pEdit = Grap_InsertEdit(pDesk, &DaoJianWT_ViewAttr[idx++], &DaoJianWT_EditPriv[i]);
		pEdit->DrawInputCnt = OpCom_EditDrawInputCnt;
		if(NULL == pEdit ) return -1;
		if(i == 0)//上一站
		{
			pEdit->editenter  = DaoJianWT_EditEnter;
			pEdit->editKeyEnd = DaoJianWT_EditKeyEnd;
			gpEdit_Stat1 = pEdit;
		}
		else if(i == 1)//重量
		{
...
/*-----------------------------------------记录查询显示------------------------------------------------*/
const U8* const opType_DaoJianWT[]=//发件
{
	"状态:",
...
*返回: 
----------------------------------------------------------------*/
int DaoJianWT_SearchDisplay(void *p)
{
...
%s%s\r\n%s%s\r\n%s%s\r\n-----------------------\r\n",
							opType_DaoJianWT[0], str[stat],
							opType_DaoJianWT[1], pComm->opType,
							opType_DaoJianWT[2], pComm->scanStat,
							opType_DaoJianWT[3], pComm->userId,
							opType_DaoJianWT[4], pComm->scanTime,
							
							opType_DaoJianWT[5], saomiao->prevstat,
							opType_DaoJianWT[6], saomiao->obj_wei,
							opType_DaoJianWT[7], pComm->code
							);
