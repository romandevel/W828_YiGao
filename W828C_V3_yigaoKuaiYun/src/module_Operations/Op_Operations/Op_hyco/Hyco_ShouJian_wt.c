...
/*==========================================收件界面==========================================*/
const tGrapViewAttr ShouJianWT_ViewAttr[] =
{
...
#endif	
	{BUTTON_ID_4,	   165,61, 70,20,    165,61,238,81,    YD_AN1_EN,YD_AN2_EN,0,0,1,0,0,0},//蓝牙
...
const tGrapButtonPriv ShouJianWT_BtnPriv[]=
{
#ifdef W818C
...
const tGrapEditPriv ShouJianWT_EditPriv[] = 
{
...
int ShouJianWT_BtnPress(void *pView, U16 state)
{
	tGRAPDESKBOX	*pDesk;
	tGRAPBUTTON 	*pBtn;
	tGRAPEDIT 		*pEditWT;
...
U32 ShouJianWT_EditKeyEnd(void *pView, U8 type)
{
	tGRAPDESKBOX	*pDesk;
...
S32 ShouJianWT_EditEnter(void *pView)
{
	tGRAPDESKBOX	*pDesk;
	tGRAPEDIT 		*pEdit;
	int ret;
...
/*-----------------------------------------------
*函数:Operation_ShouJianWT_Desk
*功能:收件
*参数:
*返回:
*-----------------------------------------------*/
int Operation_ShouJianWT_Desk(void *pDeskFather)
{
	tGRAPDESKBOX	*pDesk;
	tGRAPBUTTON		*pBtn;
	tGRAPEDIT		*pEdit;
	...
	pDesk = Grap_CreateDeskbox((tGrapViewAttr*)&ShouJianWT_ViewAttr[idx++], "收件称重");
	...
	for (i=0; i<8; i++)
	{
		pBtn = Grap_InsertButton(pDesk, &ShouJianWT_ViewAttr[idx++], &ShouJianWT_BtnPriv[i]);
		if(NULL == pBtn ) return -1;
		
		if(i < 4)
		{
			pBtn->pressEnter = ShouJianWT_BtnPress;
			if(i == 3)
			{
	...
	for (i=0; i<4; i++)
	{
		pEdit = Grap_InsertEdit(pDesk, &ShouJianWT_ViewAttr[idx++], &ShouJianWT_EditPriv[i]);
		pEdit->DrawInputCnt = OpCom_EditDrawInputCnt;
		if(NULL == pEdit ) return -1;
		if(i == 0)//收件员
		{
			pEdit->editenter  = ShouJianWT_EditEnter;
			pEdit->editKeyEnd = ShouJianWT_EditKeyEnd;
...
/*-----------------------------------------记录查询显示------------------------------------------------*/
const U8* const opType_ShouJianWT[]=//收件
{
	"状态:",
...
*返回: 
----------------------------------------------------------------*/
int ShouJianWT_SearchDisplay(void *p)
{
...
%s%s\r\n%s%s\r\n%s%s\r\n-----------------------\r\n",
							opType_ShouJianWT[0], str[stat],
							opType_ShouJianWT[1], pComm->opType,
							opType_ShouJianWT[2], pComm->scanStat,
							opType_ShouJianWT[3], pComm->userId,
							opType_ShouJianWT[4], pComm->scanTime,
							
							opType_ShouJianWT[5], saomiao->deliverer,
							opType_ShouJianWT[6], saomiao->obj_wei,
							opType_ShouJianWT[7], pComm->code
							);
...
*返回: 0:成功   -1:网络通讯异常
----------------------------------------------------------------*/
int ShouJianWT_FillData(void *p)
{
