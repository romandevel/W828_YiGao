...
/*==========================================收件界面==========================================*/
const tGrapViewAttr ShouJian_ViewAttr[] =
{
...
const tGrapButtonPriv ShouJian_BtnPriv[]=
{
#ifdef W818C
...
const tGrapEditPriv ShouJian_EditPriv[] = 
{
...
int ShouJian_BtnPress(void *pView, U16 state)
{
	tGRAPDESKBOX	*pDesk;
	tGRAPBUTTON 	*pBtn;
	tDATA_INFO 		Recd;
...
U32 ShouJian_EditKeyEnd(void *pView, U8 type)
{
	tGRAPDESKBOX	*pDesk;
...
S32 ShouJian_EditEnter(void *pView)
{
	tGRAPDESKBOX	*pDesk;
	tGRAPEDIT 		*pEdit;
	int ret;
...
/*-----------------------------------------------
*函数:Operation_ShouJian_Desk
*功能:收件
*参数:
*返回:
*-----------------------------------------------*/
int Operation_ShouJian_Desk(void *pDeskFather)
{
	tGRAPDESKBOX	*pDesk;
	tGRAPBUTTON		*pBtn;
	tGRAPEDIT		*pEdit;
	...
	pDesk = Grap_CreateDeskbox((tGrapViewAttr*)&ShouJian_ViewAttr[idx++], "收件扫描");
	...
	for (i=0; i<5; i++)
	{
		pBtn = Grap_InsertButton(pDesk, &ShouJian_ViewAttr[idx++], &ShouJian_BtnPriv[i]);
		if(NULL == pBtn ) return -1;
		
		if(i < 3)
		{
			pBtn->pressEnter = ShouJian_BtnPress;
		}
	}
	...
	for (i=0; i<3; i++)
	{
		pEdit = Grap_InsertEdit(pDesk, &ShouJian_ViewAttr[idx++], &ShouJian_EditPriv[i]);
		pEdit->DrawInputCnt = OpCom_EditDrawInputCnt;
		if(NULL == pEdit ) return -1;
		if(i == 0)//收件员
		{
			pEdit->editenter  = ShouJian_EditEnter;
			pEdit->editKeyEnd = ShouJian_EditKeyEnd;
...
/*-----------------------------------------记录查询显示------------------------------------------------*/
const U8* const opType_shoujian[]=//收件
{
	"状态:",
...
*返回: 
----------------------------------------------------------------*/
int ShouJian_SearchDisplay(void *p)
{
...
%s%s\r\n%s%s\r\n-----------------------\r\n",
							opType_shoujian[0], str[stat],
							opType_shoujian[1], pComm->opType,
							opType_shoujian[2], pComm->scanStat,
							opType_shoujian[3], pComm->userId,
							//opType_shoujian[4], pComm->groupId,
							opType_shoujian[4], pComm->scanTime,
							
							opType_shoujian[5], saomiao->deliverer,
							opType_shoujian[6], pComm->code
							);
