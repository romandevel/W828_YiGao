...
const U8 *const gcCarOrderFJZZ[]=
{
	"汽运",	"航空",	"海运"
};

/*==========================================发件界面==========================================*/
const tGrapViewAttr FaJianZZ_ViewAttr[] =
{
	...
	{EDIT_ID_20,       140,60 ,100,22,   0,0,0,0,     0,0,0xCE59,0,1,0,0,0},//中转站名称
	{EDIT_ID_21,       140,85 ,100,22,   0,0,0,0,     0,0,0xCE59,0,1,0,0,0},//下一站名称
};

const tGrapButtonPriv FaJianZZ_BtnPriv[]=
{
#ifdef W818C
...
const tGrapEditPriv FaJianZZ_EditPriv[] = 
{
...
U32 FaJianZZ_EditKeyEnd(void *pView, U8 type)
{
	tGRAPDESKBOX	*pDesk;
...
S32 FaJianZZ_EditEnter(void *pView)
{
	tGRAPDESKBOX	*pDesk;
	tGRAPEDIT 		*pEdit;
	U8 *pCode;
...
S32 FaJianZZ_ComboboxGetStr(void *pView,U16 i, U8 *pStr)
{
	strcpy(pStr, gcCarOrderFJZZ[i]);
	
	return SUCCESS;
}

S32 FaJianZZ_ComboxEnter(void *pView, U16 i)
{
...
/*-----------------------------------------------
*函数:Operation_FaJianZZ_Desk
*功能:收件
*参数:
*返回:
*-----------------------------------------------*/
int Operation_FaJianZZ_Desk(void *pDeskFather)
{
	tGRAPDESKBOX	*pDesk;
	tGRAPBUTTON		*pBtn;
	tGRAPEDIT		*pEdit;
	...
	pDesk = Grap_CreateDeskbox((tGrapViewAttr*)&FaJianZZ_ViewAttr[idx++], "发件扫描");
	...
	for (i=0; i<10; i++)
	{
		pBtn = Grap_InsertButton(pDesk, &FaJianZZ_ViewAttr[idx++], &FaJianZZ_BtnPriv[i]);
		if(NULL == pBtn ) return -1;
		
		if(i < 4)
		{
			pBtn->pressEnter = FaJianZZ_BtnPress;
			if(i == 3)
			{
	...
	pComBox = Grap_InsertCombobox(pDesk, &FaJianZZ_ViewAttr[idx++], &FaJianZZ_ComAttr[0]);
	if(NULL == pComBox ) return -1;
	pComBox->comboPriv.total_num = sizeof(gcCarOrderFJZZ)/sizeof(U8 *);
	pComBox->comboxgetstr = FaJianZZ_ComboboxGetStr;
	pComBox->comboxenter  = FaJianZZ_ComboxEnter;
	
	//中转站  下一站   重量  运单  中转站名称  下一站名称
	for (i=0; i<6; i++)
	{
		pEdit = Grap_InsertEdit(pDesk, &FaJianZZ_ViewAttr[idx++], &FaJianZZ_EditPriv[i]);
		pEdit->DrawInputCnt = OpCom_EditDrawInputCnt;
		if(NULL == pEdit ) return -1;
		if(i == 0)//中转站
		{
			pEdit->editenter  = FaJianZZ_EditEnter;
			pEdit->editKeyEnd = FaJianZZ_EditKeyEnd;
			gpEdit_Stat1 = pEdit;
		}
		else if(i == 1)//下一站
		{
...
/*-----------------------------------------记录查询显示------------------------------------------------*/
const U8* const opType_FaJianZZ[]=//发件
{
	"状态:",
...
*返回: 
----------------------------------------------------------------*/
int FaJianZZ_SearchDisplay(void *p)
{
...
%s%s\r\n%s%s\r\n%s%s\r\n%s%s\r\n-----------------------\r\n",
							opType_FaJianZZ[0], str[stat],
							opType_FaJianZZ[1], pComm->opType,
							opType_FaJianZZ[2], pComm->scanStat,
							opType_FaJianZZ[3], pComm->userId,
							opType_FaJianZZ[4], pComm->scanTime,
							
							opType_FaJianZZ[5], gcCarOrderFJZZ[atoi(saomiao->tran_types)],
							opType_FaJianZZ[6], saomiao->prevstat,
							opType_FaJianZZ[7], saomiao->nextstat,
							opType_FaJianZZ[8], pComm->code
							);
