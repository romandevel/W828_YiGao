...
/*-----------------------------------------------------------------------------
* 函数:	calc_next_expiration
* 功能:	re-set kernel timer
* 参数:	none
* 返回:	none
*----------------------------------------------------------------------------*/
...
retry:
	...
	llist_for_each_entry(cur, struct gsmd_timer, &gsmd_timers, list) {
...
void gsmd_timer_check_n_run(void)
{
	...
	llist_for_each_entry_safe(cur, cur2, struct gsmd_timer, &gsmd_timers, list) {
...
struct gsmd_timer *gsmd_timer_alloc(void)
{
	...
	tmr = (struct gsmd_timer *)MALLOC(sizeof(*tmr));
	return tmr;
}
