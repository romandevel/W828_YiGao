...
struct gsm_extrsp *extrsp_parse(const char *input)
{
	...
	er = (struct gsm_extrsp *)MALLOC(sizeof(struct gsm_extrsp));
	if (!er)
		return NULL;
	...
	while (*cur) {
		...
		case IDLE:
			...
			if (isblank(*cur))
				...
				state = TOKEN_RANGE;
			} else if (isdigit(*cur)) {					//c standard fuction
...
void extrsp_dump(const struct gsm_extrsp *er)
{
	...
	for (i = 0; i < er->num_tokens; i++) {
		...
		case GSMD_ECMD_RTT_RANGE: {
			...
			//	DEBUGP("%d-%d, ", tok->u.range.item[j].min,	tok->u.range.item[j].max);
			}
			break;
		}
	}
