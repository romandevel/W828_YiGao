...
int sms_pdu_to_msg(struct gsmd_sms_list *dst,
		const u_int8_t *src, int pdulen, int len, int ds)
{
	int i, vpf;
	...
	case GSMD_SMS_TP_MTI_DELIVER:
		...
		case 0x0:	/* General Data Coding indication */
		case 0x1:	/* General Data Coding indication */
		case 0x2:	/* General Data Coding indication */
		case 0x3:	/* General Data Coding indication */
	...
	case GSMD_SMS_TP_MTI_SUBMIT:
		...
		//如果有效期格式TP-VPF（TP-Validity-Period-Format）为00 --- 不提供（Not present）
		if (len < (vpf ? 8 : 7) + i)
			return 1;
