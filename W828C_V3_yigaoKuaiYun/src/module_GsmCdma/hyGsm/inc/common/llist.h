...
#if 0
#define container_of(ptr, type, member) ({			\
        const struct llist_head *__mptr = (ptr);	\
        (type *)( (char *)__mptr - offsetof(type,member) );})
#endif
 ...
 * @member:	the name of the llist_struct within the struct.
 */
#define llist_for_each_entry(pos, type, head, member)		\
 ...
 * @member:	the name of the llist_struct within the struct.
 */
#define llist_for_each_entry_safe(pos, n, type, head, member)		\
	for (pos = llist_entry((head)->next, type, member),				\
		n = llist_entry(pos->member.next, type, member);			\
	     &pos->member != (head); 									\
	     pos = n, n = llist_entry(n->member.next, type, member))
