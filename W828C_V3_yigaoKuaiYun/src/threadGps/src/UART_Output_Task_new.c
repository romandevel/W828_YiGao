...
#include "Baseband_Patch_510_P076.c"
            ...
            if ( num > 0 )
            {
               ...
               remain_len = num;
				      // Send the data
				       do 
				       {
				       		S_Len = UART_PutBytes(GPS_UART_PORT, (kal_uint8 *)UARTtx_cb.read+(num-remain_len), remain_len);
				       		if (S_Len < 0)
				       		{
				       				//re-send
  ...
  const U1 *p_Comm;       // Pointer to the array of sentence identifiers to send
   ...
   while ( (num_sent < PATCH_SPEED) && (cur_mess[index] < tot_num) )
   {
      ...
      // Send the data
       do 
       {
       		S_Len = UART_PutBytes(GPS_UART_PORT, (kal_uint8 *)patch_data+(size-remain_len), remain_len);
       		if (S_Len < 0)
       		{
       				//re-send
